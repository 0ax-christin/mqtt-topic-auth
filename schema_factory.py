...
def generate_signed_ticket(private_key: Ed25519PrivateKey, ticket_id:str, device_id: str, public_key:bytes,
                           expiry: int, seed: bytes, hmac_key: bytes) -> signed_ticket:
    new_ticket = ticket.new_message(ticketId=ticket_id, deviceId=device_id,
                                    publicKey=public_key, expiry=expiry,
                                    seed=seed, hmacKey=hmac_key)
    ticket_bytes = new_ticket.to_bytes_packed()
    signature = private_key.sign(hashlib.sha3_256(ticket_bytes).digest())
    signed_new_ticket = signed_ticket.new_message(ticket=new_ticket, signature=signature)
    return signed_new_ticket

def generate_signed_token(private_key: Ed25519PrivateKey, ticket_id:str, mqtt_topic: str,
                          mqtt_username: str, hmac_key: bytes, seed: bytes) -> signed_token:
    new_token= token.new_message(ticketId=ticket_id, mqttTopic=mqtt_topic,
                                 mqttUsername=mqtt_username, hmacKey=hmac_key,
                                 seed=seed)
    token_bytes = new_token.to_bytes_packed()
    signature = private_key.sign(hashlib.sha3_256(token_bytes).digest())
...
def generate_request_bytes(requestType: str, nonce: bytes = b'', solution: bool = False) -> bytes:
